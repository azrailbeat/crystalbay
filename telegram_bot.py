...
async def initiate_booking(query, tour_id):
    ...
    if not selected_tour:
        await query.message.reply_text("😔 Информация о выбранном туре не найдена. Пожалуйста, начните поиск заново.")
        return
...
async def process_booking_input(update: Update, context: CallbackContext):
    ...
    if user_id not in user_sessions:
        await update.message.reply_text("Сессия истекла. Пожалуйста, начните поиск заново с команды /start")
        return
    ...
    if booking_stage == "name":
        ...
        await update.message.reply_text("Спасибо! Теперь укажите ваш номер телефона для связи:")
...
async def finalize_booking(query):
    """Complete the booking process"""
    user_id = query.from_user.id
    
    if user_id not in user_sessions:
        await query.message.reply_text("Сессия истекла. Пожалуйста, начните поиск заново с команды /start")
        return
...
async def handle_text_message(update: Update, context: CallbackContext):
    ...
    if user_id in user_sessions and "booking_stage" in user_sessions[user_id]:
        ...
        await process_booking_input(update, context)
    else:
