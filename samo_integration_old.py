    ...
    def _make_request(self, action: str, params: Dict = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # Если получаем 403 (IP заблокирован), возвращаем ошибку
                if response.status_code == 403:
                    logger.error(f"SAMO API access blocked for {action}")
                    return {
                        'success': False,
            ...
            # В случае ошибки сети возвращаем ошибку
            logger.error(f"Network error for {action}: {e}")
            return {
                'success': False,
